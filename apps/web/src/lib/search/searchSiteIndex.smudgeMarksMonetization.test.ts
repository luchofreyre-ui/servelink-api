...
describe("searchSiteIndex smudge-marks monetization", () => {
  it("stacks authority problem, compare, then best scenario product", () => {
    const results = searchUnifiedDocuments("smudge marks", { limit: 24 });

    expect(results[0]?.href).toBe("/problems/smudge-marks");

...
const PROBLEMS_DESCRIPTION = "Problem definitions linked to methods, surfaces, and combo playbooks.";

export default function ProblemsIndexPage() {
  const problems = getAllProblemPages();
  const indexJsonLd = [
    buildBreadcrumbListSchema([
      { label: "Home", href: "/" },
  ...
  return (
    <div className="min-h-screen bg-[#FFF9F3] text-[#0F172A]">
      ...
      <main className="mx-auto max-w-3xl px-6 py-16 md:px-8">
        ...
        <h1 className="mt-6 font-[var(--font-poppins)] text-4xl font-semibold tracking-tight">{PROBLEMS_TITLE}</h1>
        <p className="mt-4 font-[var(--font-manrope)] text-lg leading-8 text-[#475569]">{PROBLEMS_DESCRIPTION}</p>
        <ul className="mt-10 space-y-3 font-[var(--font-manrope)] text-sm">
          {problems.map((p) => (
            <li key={p.slug}>
              <Link href={`/problems/${p.slug}`} className="font-medium text-[#0D9488] hover:underline">
                {p.title}
              </Link>
            </li>
          ))}

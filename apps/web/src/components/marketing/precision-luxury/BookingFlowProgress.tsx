...
export function BookingFlowProgress({
  ...
  return (
    <div className="rounded-[18px] border border-[#C9B27C]/16 bg-white p-2.5 shadow-[0_18px_50px_rgba(15,23,42,0.05)] sm:rounded-[28px] sm:p-6">
      <div className="-mx-1 flex gap-2 overflow-x-auto px-1 pb-1 sm:gap-4 lg:grid lg:grid-cols-5 lg:overflow-visible lg:pb-0">
        {steps.map((step, index) => {
          ...
          return (
            <div key={step.id} className="flex min-w-[7.25rem] flex-1 items-center gap-2 sm:min-w-0 sm:gap-3">
              <div
                className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full border text-[11px] font-semibold font-[var(--font-manrope)] sm:h-11 sm:w-11 sm:text-sm ${NU_PREMIUM_TRANSITION} ${nuPremiumFocusRing} ${
                  isActive
              ...
              <div className="min-w-0">
                <p
                  className={`truncate font-[var(--font-manrope)] text-[11px] font-medium sm:text-sm ${

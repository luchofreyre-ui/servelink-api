...
import type { SystemTestIncidentEvent, SystemTestIncidentEventType } from "@/types/systemTestIncidentActions";

function eventLabel(type: SystemTestIncidentEventType): string {
  const map: Record<SystemTestIncidentEventType, string> = {
    ...
    incident_seen: "Incident seen in run",
...
function formatMetadataSummary(
  ...
  switch (type) {
    ...
    case "unassigned": {
      ...
      const b = before === null || before === undefined ? "Unassigned" : String(before);
      const a = after === null || after === undefined ? "Unassigned" : String(after);
      return `${b} → ${a}`;
    }
    case "reopened": {
      const reason = r("reason");
    ...
    case "incident_seen": {
      const runId = r("runId");
      const prev = r("previousLastSeenRunId");
      const st = r("currentStatus");
      const parts = [
        typeof runId === "string" ? `Run: ${runId}` : "",
        prev != null ? `Previous run: ${String(prev)}` : "",
    ...
    case "notification_queued": {
      ...
      const parts = [
        ...
        typeof pri === "string" ? `Priority: ${pri}` : "",
        typeof st === "string" ? `Status: ${st}` : "",
        owner === null ? "Owner: unassigned" : owner !== undefined ? `Owner: ${String(owner)}` : "",
...
export function SystemTestIncidentTimeline({ events }: Props) {
  ...
  if (!ordered.length) {
    return (
      <p className="text-sm text-white/45">No timeline events yet.</p>
    );
  }
  ...
  return (
    <ul className="space-y-4">
      {ordered.map((ev) => {
        const meta =
          ev.metadataJson && typeof ev.metadataJson === "object" && !Array.isArray(ev.metadataJson)
        ...
        return (
          ...
          >
            ...
            {summary ?
              <div className="mt-1 font-mono text-[11px] text-white/55">{summary}</div>
            : null}
          </li>
        );
      })}

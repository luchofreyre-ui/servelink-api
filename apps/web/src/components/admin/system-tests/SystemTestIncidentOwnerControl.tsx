...
export function SystemTestIncidentOwnerControl({
  incidentKey,
  ownerUserId,
  disabled,
  onUpdated,
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
      ...
      <p className="mt-1 text-xs text-white/50">
        Paste a user id to assign. Current: {ownerUserId ? <span className="font-mono text-white/70">{ownerUserId}</span> : "Unassigned"}
      </p>
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            onClick={async () => {
              ...
              setSaving(true);
              try {
                await updateOwner({ incidentKey, ownerUserId: null });
                setInput("");
                await onUpdated();
              } catch (e) {

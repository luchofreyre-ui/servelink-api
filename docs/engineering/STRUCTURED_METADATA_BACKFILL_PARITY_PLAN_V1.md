...
## 9. Implementation note — dry-run CLI (V1)

| Item | Detail |
|------|--------|
| **CLI** | `npm run backfill:booking-operational-metadata:dry-run` → `services/api/scripts/backfill-booking-operational-metadata-dry-run.ts` |
| **Core lib** | `services/api/scripts/backfill-booking-operational-metadata-dry-run.lib.ts` — pure classification + safe report builders |
